...
pcaServer <- function(input, output, session, meta_data, counts_data, deseq_data) {
    ...
    output$axis <- renderUI({
        ...
        tagList(
            ...
            numericInput(session$ns("x_min"), "X min", value = -50),
            numericInput(session$ns("x_max"), "X max", value =  50),
            numericInput(session$ns("y_min"), "Y min", value = -50),
            numericInput(session$ns("y_max"), "Y max", value =  50)
        )
    })
