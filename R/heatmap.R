        ...
        sidebarLayout(
            sidebarPanel(
                ...
                radioButtons(ns("filter_column"), "Filter DE genes by:", 
                             choices = c("padj" = "padj", "pvalue" = "pvalue"), selected = "padj"),
            ...
            mainPanel(
                ...
                withSpinner(plotOutput(ns("heatmap_plot"), height = "800px"))
            )
        )
    )
}
...
heatmapServer <- function(id, meta_data, counts_data, deseq_data) {
    moduleServer(id, function(input, output, session) {
        ...
        clean_data <- reactive({
            ...
            # A. Fix Counts Gene ID (Target name: "gene_name")
            if (!"gene_name" %in% colnames(counts)) {
                ...
                if (length(found) > 0) {
                    ...
                    setnames(counts, colnames(counts)[1], "gene_name") # Fallback to 1st col
                }
            }
            
            # B. Fix Metadata Sample ID (Target name: "sampleID")
            if (!"sampleID" %in% colnames(meta)) {
                candidates <- c("Sample", "SampleID", "id", "ID", "name", "Name")
                found <- intersect(colnames(meta), candidates)
                if (length(found) > 0) {
                    setnames(meta, found[1], "sampleID")
                } else {
            ...
            if (!"log2FoldChange" %in% colnames(deseq)) {
                if ("logFC" %in% colnames(deseq)) setnames(deseq, "logFC", "log2FoldChange")
            }
            # P-values
            if (!"padj" %in% colnames(deseq) && "adj.P.Val" %in% colnames(deseq)) setnames(deseq, "adj.P.Val", "padj")
            ...
            de_id_col <- "gene_name"
            if (!"gene_name" %in% colnames(deseq)) {
                candidates <- c("GeneID", "Geneid", "gene_id", "id", "ID", "row", "Row.names")
                found <- intersect(colnames(deseq), candidates)
        ...
        heatmap_prep <- reactive({
            ...
            if (!filter_col %in% colnames(deseq)) return(list(error = paste("Column", filter_col, "missing in DESeq results")))
            ...
            if (length(sig_genes) == 0) return(list(error = "No significant genes found. Relax thresholds."))
            
            # Subset Counts (Genes)
            counts_filtered <- counts[ gene_name %in% sig_genes ]
        ...
        # --- 3. Build Heatmap Object ---
        build_heatmap <- reactive({
            prep <- heatmap_prep()
            if (is.null(prep) || !is.null(prep$error)) return(NULL)
            ...
            # Splits
            col_split_val <- if(input$mode == "supervised" && "Group" %in% colnames(ann)) ann$Group else if(input$mode == "unsupervised") input$col_split else NULL
            cluster_cols <- if(input$mode == "supervised") FALSE else TRUE
            
            Heatmap(
                m,
                name = if (isTRUE(input$scale_rows)) "Z-score" else "Value",
                col = my_col,
                use_raster = FALSE, # Set to TRUE for very large datasets
        ...
        output$heatmap_plot <- renderPlot({
            ht <- build_heatmap()
            validate(need(!is.null(ht), "Waiting for data..."))
        ...
        output$download_heatmap <- downloadHandler(
            ...
            content = function(file) {
                ht <- build_heatmap()
                # Use grid.grabExpr to capture ComplexHeatmap output
                gr <- grid::grid.grabExpr(draw(ht, merge_legend = TRUE))
                # Convert to ggplot for easy saving with ggsave
                gg_obj <- as.ggplot(gr)
                ggsave(file, plot = gg_obj, width = 10, height = 10, dpi = 300)
            }
        )
